...
export default function AdminReservationActions({ id, code, overdue }: { id: string; code: string; overdue: boolean }) {
  ...
  return <div className="sans">
    {action ? <form onSubmit={submit} className="rounded-xl bg-slate-50 p-4">
      ...
      <div className="mt-4 flex flex-wrap gap-3"><button disabled={busy} className="rounded-xl bg-[#101828] px-4 py-3 text-sm font-bold text-white disabled:opacity-50">{busy ? "Guardando..." : action === "confirm" ? "Sí, confirmar pago" : "Sí, liberar números"}</button><button type="button" disabled={busy} onClick={() => { setAction(null); setError(""); }} className="rounded-xl border border-slate-200 px-4 py-3 text-sm font-bold">Cancelar</button></div>
